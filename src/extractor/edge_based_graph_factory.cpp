    ...
    const bool generate_edge_lookup)
{
    ...
    for (const auto node_u : util::irange(0u, m_node_based_graph->GetNumberOfNodes()))
    {
        ...
        for (const EdgeID edge_from_u : m_node_based_graph->GetAdjacentEdgeRange(node_u))
        {
            ...
            for (const auto turn : possible_turns)
            {
                ...
                ExtractionTurn extracted_turn(guidance::isUturn(turn.instruction), 180. - input_turn.angle);
                scripting_environment.ProcessTurn(extracted_turn);
                if (fallback_to_duration && extracted_turn.weight < 0 && extracted_turn.duration > 0)
                {
                    turn.weight = turn.duration * 10.; // convert from seconds in deci-seconds
                }
                weight += std::max(0, extracted_turn.weight);
                duration += std::max(0, extracted_turn.duration);
    ...
    std::ofstream turn_weight_penalties_file(turn_weight_penalties_filename.c_str(), std::ios::binary);
    ...
    turn_weight_penalties_file.write(reinterpret_cast<const char *>(turn_weight_penalties.data()),
                              sizeof(decltype(turn_weight_penalties)::value_type) * turn_weight_penalties.size());
    // write out the duration penatlies per durn if we need them
    std::ofstream turn_duration_penalties_file(turn_duration_penalties_filename.c_str(), std::ios::binary);
    io::TurnPenaltiesHeader turn_duration_penalties_header {turn_duration_penalties.size()};
    turn_duration_penalties_file.write(reinterpret_cast<const char *>(&turn_duration_penalties_header),
                              sizeof(turn_duration_penalties_header));
    if (!fallback_to_duration)
    {
        BOOST_ASSERT(turn_weight_penalties.size() == turn_duration_penalties.size());
        turn_duration_penalties_file.write(reinterpret_cast<const char *>(turn_duration_penalties.data()),
